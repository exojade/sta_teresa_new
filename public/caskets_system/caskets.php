...
use mikehaertl\pdftk\Pdf;
    if($_SERVER["REQUEST_METHOD"] === "POST") {
		...
		$caskets = query("select * from casket");
		$caskets_image = query("select * from casket_image");

		$Caskets_image = [];
		foreach($caskets_image as $c):
			$Caskets_image[$c["casket_id"]][$c["casket_image_id"]] = $c;
		endforeach;


		if($_GET["action"] == "list"){
			render("public/caskets_system/caskets_list.php", [
				"title" => "Caskets Databank",
				"caskets" => $caskets,
				"caskets_image" => $caskets_image,
				"Caskets_image" => $Caskets_image,
			]);
		}
